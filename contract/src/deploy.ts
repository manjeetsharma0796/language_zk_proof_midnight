...
import { Contract, ledger as languageLedger } from './managed/language/contract/index.js';
...
// ---------------------------------------------------------------------------
// Config – fixed to match midnight-local-network Docker expose (do not change)
// indexer 8088, node 9944, proof-server 6300. Override via env if needed.
// ---------------------------------------------------------------------------
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'language');
...
type LanguagePrivateState = Record<string, never>;
type LanguageCircuits = ImpureCircuitId<Contract<LanguagePrivateState>>;
const LanguagePrivateStateId = 'languagePrivateState' as const;
...
async function main() {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║           Language Contract Deploy Script               ║');
  ...
  const zkConfigProvider = new NodeZkConfigProvider<LanguageCircuits>(zkConfigPath);

  const providers = {
    privateStateProvider: levelPrivateStateProvider<typeof LanguagePrivateStateId>({
      privateStateStoreName: 'language-private-state',
  ...
  // --- Compile contract ---
  const languageCompiledContract = CompiledContract.make('language', Contract).pipe(
  ...
  // --- Deploy ---
  const deployOptions = {
    compiledContract: languageCompiledContract,
    privateStateId: LanguagePrivateStateId,
    initialPrivateState: {},
  };
  const contract = await withStatus('Deploying language contract (this may take a few minutes)', async () =>
    deployContract(providers, deployOptions as unknown as Parameters<typeof deployContract>[1]),
  );
  ...
  console.log(`\n╔══════════════════════════════════════════════════════════╗`);
  console.log(`║  Language Contract deployed successfully!               ║`);
  console.log(`╚══════════════════════════════════════════════════════════╝`);
